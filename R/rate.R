...
{
  endpoint <- "/v1/candles"
  query <- list(instrument=instrument, granularity=granularity, count=count, start=start, end=end, candleFormat=candle_format, includeFirst=include_first, dailyAlignment=daily_alignment, alignmentTimezone=alignment_timezone, weeklyAlignment=weekly_alignment)
  request(oanda, endpoint, method=GET, params=list(query=query))
}
